...
- [x] 사이드바 다크 배경 (`#000000`)
- [x] 컨텐츠 다크 배경 (`#1C1C1E`)
- [x] 카드/모달 다크 배경 (`#2C2C2E`)
- [x] 텍스트/구분선 다크 색상 적용
- [x] 프리셋 색상 Light/Dark 값 분기

### 6.2 애니메이션
- [x] 완료: 원 채움 → 체크마크 → 0.5초 후 페이드아웃 + 슬라이드업
- [x] 삭제: 좌로 슬라이드아웃 + 페이드
- [x] 디테일 패널: 우측 슬라이드-인/아웃 (200ms ease)
- [x] 리스트 전환: 크로스페이드 (150ms)
- [x] 모달: 페이드 + 스케일 (0.95 → 1.0)
- [x] 스마트 리스트 카드: hover brightness 1.05

### 6.3 반응형
- [x] 태블릿 (< 1024px): 사이드바 오버레이 + 햄버거 토글
- [x] 모바일 (< 768px): 사이드바 → 목록 → 디테일 단계별 내비게이션
- [x] 모든 터치 타겟 최소 44px 보장

### 6.4 마무리
- [ ] 리마인더 행 우클릭 컨텍스트 메뉴 (완료, 깃발, 삭제, 리스트 이동)
- [x] 키보드 단축키 (Enter=추가, Delete=삭제, Escape=패널 닫기)
- [x] API 실패 시 토스트 에러 알림
- [x] 로딩 상태 스켈레톤 UI
- [x] 초기 데이터 시딩 (첫 실행 시 샘플 리스트/리마인더 자동 생성)
